...
def python_board():
    pyd=Doska()
    pyd.set_ships([Ship.points_for_pyship(1,3, pyd.busy)])
    name_count=2
    for i in range (2):
        pyd.set_ships([Ship.points_for_pyship(name_count,2, pyd.busy)])
        name_count+=1
    for i in range (4):
        pyd.set_ships([Ship.points_for_pyship(name_count,1, pyd.busy)])
        name_count+=1
    return pyd
...
if __name__ == "__main__":
    ...
    Python_d=python_board() #инициализируем доску Питона
